...
백엔드가 없는 정적 사이트라 Vercel에 그대로 올라갑니다. 저장소를 연결하면 `vercel.json`의
설정(빌드 명령, 출력 디렉터리 `dist`, SPA 리라이트, 캐시 헤더)이 그대로 적용됩니다.

```bash
pnpm build   # dist/ 생성 (index.html, assets/, sw.js, manifest.webmanifest)
```

### PWA (앱으로 설치)

- 브라우저 메뉴의 **홈 화면에 추가 / 설치**로 앱처럼 쓸 수 있습니다
- 서비스워커가 앱 전체를 캐시하므로 **오프라인에서도 열립니다**
- 설치해두면 iOS Safari의 7일 미사용 저장소 삭제 대상에서 제외돼 데이터가 더 안전합니다
- 새 버전이 배포되면 자동 새로고침 대신 토스트로 물어봅니다 (편집 중 내용 보호)
...
- 브라우저 데이터 삭제 시 모든 저장된 데이터 손실
  (첫 저장 시 `navigator.storage.persist()`로 자동 삭제를 막지만, 사용자가 직접 지우면 복구 불가)
  → 중요한 작업은 `.vcf`로 내보내 백업해두세요
